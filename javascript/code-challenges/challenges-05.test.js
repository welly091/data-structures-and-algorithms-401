...
const reversedString = (str) => {
  //Create an array to store string's characters 
  const strArray = str.split("")
  let index = strArray - 1
  //Iterate array and reverse add too accumulator
...
const countNumberOfChildren = (arr) => {
  return arr.reduce((accumulator, currentValue) =>{
    if(currentValue.children) return accumulator + currentValue.children.length
    return accumulator
  },0)
};
...
const countPrimeNumbers = (arr) => {
  return arr.reduce((accumulator,currentValue) =>{
    if(isPrime(currentValue))return accumulator + 1
    return accumulator
  },0)
};
...
const extractChildren = (arr) => {
  return arr.filter(person => person.name.includes('a'))
            .reduce((accumulator, currentValue) =>{
              if(currentValue.children){
                currentValue.children.forEach(child => accumulator.push(child))
              }
              return accumulator
            },[])
};
