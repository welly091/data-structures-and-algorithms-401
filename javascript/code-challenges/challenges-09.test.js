...
const checkValues = (obj, value) => {
  let result = false;
  Object.values(obj).forEach(v =>{ if(v === value) result = true })
  return result;
};
...
const hasChildrenValues = (arr, character) => {
  ...
  //       if(key === ' character'){
  //         kids = Object.values(person[index]).length
  //       }
  //     })
  //   }
  // })
  // return kid;
   return (arr.filter(obj => obj.name === character && obj.children).length > 0)
...
const hasChildrenEntries = (arr, character) => {
  return arr.filter(obj => Object.entries(obj)[0][1] === character && Object.entries(obj)[2][0] === 'children').length > 0
};
...
const totalCharacters = (arr) => {
  ...
  arr.forEach(obj => {
    for (const [key, value] of Object.entries(obj)){
      if(key === 'children') result = result + value.length
      else if(key != 'house' && value != null) result++;
    }
  })
  return result
};
...
const houseSize = (arr) => {
  ...
  arr.forEach(fam =>{
    let count = 0
    Object.entries(fam).forEach(([key,value]) =>{
      if(key === 'children') count=count+value.length
      else if(key != 'house' && value != null) count++
    })
    sizes.push({house: fam.house, members: count})
  })
  return sizes;
};
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach(fam =>{
    ...
    Object.entries(fam).forEach(([key,value]) =>{
      if(key === 'children') count=count+value.length
      else if(key === 'spouse' && value != null && !deceasedSpouses.includes(value)) count++
      else if(key === 'name') count++
    })
    survivors.push({house: fam.house, members: count})
  })
  return survivors;
};
