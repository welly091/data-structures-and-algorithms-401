...
const isCapitalized = (str) => {
  return str.split(/\W/).filter(element => element.match(/^[A-Z]/))
};
...
const citiesAtoJ = (arr) => {
  return arr.filter(city => city.match(/^[A-J]/))
};
...
const matchMonth = (input) => {
  const oct1 = RegExp(/^October$/)
  const oct2 = RegExp(/^Oct$/)
  const oct3 = RegExp(/^october$/)
  const oct4 = RegExp(/^oct$/)
  return (oct1.test(input) || oct2.test(input) || oct3.test(input) || oct4.test(input))
...
const noPunctuation = str => {
  ...
  strArray.forEach(word =>{
    if(word.match(/^\w+\w$/) || word.match(/^\w$/)) result.push(word + " ")
  })
  return result
};
...
const findShells = (str) => {
  ...
  strArray.forEach(element => {
    if(element.match(/sells/) || element.match(/shells/) || element.match(/seashells/)) result.push(element)
  })
  return result
};
