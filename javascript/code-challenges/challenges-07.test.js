...
const listFoods = (recipe) => {
  ...
  //Use while loop. Time:O(m x n)
  recipe.ingredients.forEach(element => {
    let index = 0;
    while(element.length){
      if(index == 2) break
      if(element.substring(0,1) === " ")index++
...
const splitFoods = (recipe) => {
  ...
  recipe.ingredients.map(element => {
    ...
    result.push(temp.splice(2).join(' '))
  })
  return result;
};
...
const totalSumCSV = (str) => {
  let total = 0;
  total = str.split(",").reduce((a,c) =>  a + parseInt(c), 0)
  return total;
};
...
const removeVowels = (str) => {
  ...
  for(let i  = 0; i < str.length; i++){
    if(str[i] !== 'i' && str[i] !== 'e' && str[i] != 'a' && str[i] !== 'o' && str[i] !== 'u'){
      newStr=newStr+str[i]
    }
  }
  return newStr
...
const extractVowels = (str) => {
  ...
  return [newStr,onlyVowels.split('').sort().join('')]
};
