...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(e => e.filter( ele => typeof ele =='number' && ele % 5 == 0)).map(e => e.map(ele => Math.pow(2,ele)))
...
let findMaleAndFemale = (data) => {
  return data.filter(obj => (obj.gender === 'female' || obj.gender === 'male')).map(obj => obj.name).reduce((a,c) =>
    a + ' and ' + c
  )
};
